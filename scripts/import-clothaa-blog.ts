...
async function main() {
  const dataPath = path.join(__dirname, 'data', 'clothaa-blog-import.json')
    ...
    const result = await BlogPost.findOneAndUpdate(
      { slug: finalSlug },
      {
        $set: {
          ...
          status: finalStatus,
          publishedAt,
          author: { name: p.authorName || 'Clothaa Team' },
          seoTitle: p.seoTitle,
  ...
  if (unresolvedImages.length) {
    console.log(`\n${unresolvedImages.length} post(s) have a coverImage that won't resolve on our domain (relative Clothaa asset paths) — featuredImage left empty, needs a manual upload via the admin blog editor:`)
